    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                IdentityResult result = await UserService.ResetPasswordAsync(user.Id, model.Code, model.Password);
                if (result.Succeeded)
                {
        ...
        public async Task<ActionResult> Disassociate(string loginProvider, string providerKey)
        {
            ...
            IdentityResult result = await UserService.RemoveLoginAsync(User.Identity.GetUserId(), new UserLoginInfo(loginProvider, providerKey));
            if (result.Succeeded)
            {
        ...
        public async Task<ActionResult> Manage(ManageUserViewModel model)
        {
            ...
            if (hasPassword)
            {
                if (ModelState.IsValid)
                {
                    IdentityResult result = await UserService.ChangePasswordAsync(User.Identity.GetUserId(), model.OldPassword, model.NewPassword);
                    if (result.Succeeded)
                    {
                        var user = await UserService.FindByIdAsync(User.Identity.GetUserId());
                        await SignInAsync(user, isPersistent: false);
        ...
        public async Task<ActionResult> LinkLoginCallback()
        {
            ...
            IdentityResult result = await UserService.AddLoginAsync(User.Identity.GetUserId(), loginInfo.Login);
            if (result.Succeeded)
            {
        ...
        {
            var linkedAccounts = UserService.GetLogins(User.Identity.GetUserId());
